...
export function initServer(
	...
	signer: Signer
): [ExecutionManager, EventsManager, ReputationManager, MempoolManager, IValidationManager] {
	...
	if (!config.useRip7560Mode) {
		...
		bundleManager = new BundleManagerRIP7560(
			entryPoint.provider as JsonRpcProvider,
			signer,
			eventsManager,
			mempoolManager,
	...
	return [executionManager, eventsManager, reputationManager, mempoolManager, validationManager];
}

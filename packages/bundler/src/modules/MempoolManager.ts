...
export class MempoolManager {
	...
	private checkMultipleRolesViolation(userOp: OperationBase): void {
		...
		requireCond(
			...
			`A Paymaster at ${
				paymaster as string
			} in this UserOperation is used as a sender entity in another UserOperation currently in mempool.`,
			ValidationErrors.OpcodeValidation
		);
		requireCond(
	...
	private checkReputationStatus(
		...
		const maxTxMempoolAllowedEntity = this.reputationManager.calculateMaxAllowedMempoolOpsUnstaked(
			title,
			stakeInfo.addr
		);

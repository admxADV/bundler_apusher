...
export class ValidationManager implements IValidationManager {
	...
	async validateUserOp(
		...
		requireCond(
			res.returnInfo.validUntil == null || res.returnInfo.validUntil > now + VALID_UNTIL_FUTURE_SECONDS,
			"expires too soon",
			ValidationErrors.NotInTimeRange
		);
	...
	async getCodeHashes(addresses: string[]): Promise<ReferencedCodeHashes> {
		const { hash } = await runContractScript(this.entryPoint.provider, new CodeHashGetter__factory(), [addresses]);
	...
	validateInputParameters(
		...
		if ((userOp as UserOperation).preVerificationGas != null) {
			...
			requireCond(
				BigNumber.from((userOp as UserOperation).preVerificationGas).gte(calcPreVerificationGas1),
